...
export class Field {
  ...
  /**
   * Query the playfield shell only, ignoring the goal recesses. Used to build
   * the visual mesh, which is generated by projecting vertices onto this exact
   * surface -- so what you see is precisely what you collide with.
   */
  queryShell(p: Vec3): SurfaceQuery {
    return this.arena.query(p);
  }

  /**
   * Distance from an interior point to the shell along `dir`. The playfield is
   * convex, so a ray from inside crosses the boundary exactly once and a
   * bisection is both exact and cheap.
   */
  shellDistance(origin: Vec3, dir: Vec3, maxDist = 20000): number {
    let lo = 0;
    let hi = maxDist;
    // Expand until we are outside, then bisect.
    for (let i = 0; i < 40; i++) {
      const mid = (lo + hi) * 0.5;
      if (this.queryShell(V.addScaled(origin, dir, mid)).dist > 0) lo = mid;
      else hi = mid;
    }
    return (lo + hi) * 0.5;
  }
